...
class AuthController {
    ...
    async login(req, res) {
        ...
        try {
            ...
            const user = await this.User.findOne({ $or: [{ email }, { mobile }] });
            ...
            await this.User.findOneAndUpdate({ _id: user._id }, { $set: { token: token, status: 'active' } }, { new: true });
    ...
    async getUser(req, res) {
        try {
            ...
            const user = await this.User.findById(userId).populate("roleId").lean();
